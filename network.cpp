...
//initializes all of the network’s information from file name; and returns 0 on open and -1 on success
...
//writes the network's information to the file 
int Network::write_friends(const char *file) {
	ofstream out_file(file);
	if(!out_file.is_open()) { 
		return -1;    //returns -1 if not opened
	}
	else {
...
//adds user to network database
void Network::add_user(string username, int yr, int zipcode, vector <int> friend_list) {
	network.push_back(User(network.size(), username, yr, zipcode, friend_list));
}

//acceps user (friend) name to be added to friend network
...
int Network::remove_connection(string name1, string name2) {
  int id1, id2;
	...
	else if (network[id1].get_name() == name1 && network[id2].get_name() == name2) {
...
//accepts user's name and returns user id, else -1
...
//function returns the current size of the network, will be used in social_network.cpp 
int Network::get_size() {
	return network.size();
}

//returns user based on specified network index
User Network::get_user(int index) {
	return network[index];
}

//shortest past starting at "from" and ending at "to," and returns empty vector if none exists 
vector<int> Network::shortest_path(int from, int to) {
	vector<int> path;
	bfs(from);
	if(network[to].get_depth() == -1) { //returns if nonexistent 
		return path; 
	}
	else {
	  int x = to;
...
//list of the components of the network 
...
//suggests friends with highest score for this user 
...
vector<int> Network::bfs(int id) {
	vector<int> q;
	vector<int> visited; //represents users already parsed in bfs 
	q.push_back(id);    //adds ids to vector
	visited.push_back(id);    //adds ids to vector
